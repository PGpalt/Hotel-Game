...
public class BuildDice : MonoBehaviour
{
    private float force;
    ...
    void Update()
    {
        ...
        started ++;  // begin after a few frames so that the dice has started moving;
    }

    public void wait()
    {
        GameLogic.normalDice.SetActive(true);
        GameLogic.buildDice.SetActive(false);
        gameObject.transform.parent.gameObject.SetActive(false);
        GameLogic.BuildPanel.SetActive(true);
        GameLogic.BuildButton.SetActive(false);
        GameLogic.SwapCamToMain();
    }
}
